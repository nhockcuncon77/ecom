...
UNDERPER_REPORT       = "underperforming_ats_report_V2.xlsx"
...
unit_df = pd.read_excel(ORDER_PATH, sheet_name="Unit Financial", skiprows=3)  # Planned per-unit price
...
def tool_orders_below_plan_for_asin(prompt: str) -> str:
    ...
    # planned unit price
    plan_ser = (
        unit_df.loc[unit_df["ASIN"].astype(str).str.upper() == asin_q, "Amazon Top-line Sales (ATS)"]
        .dropna()
        .astype(str)
    )
    if plan_ser.empty:
...
def tool_gross_margin_underperformers(_: str) -> str:
    ...
    try:
        ...
        gm_under = pd.read_excel("GM_Underperformers.xlsx")
